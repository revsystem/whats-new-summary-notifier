...
* `responses-runtime` では bedrock-mantle の権限付与は行いません。ベアラートークンは SigV4 の presigned URL をローカルで生成したものなので、モデルに対する `bedrock:InvokeModel` と `bedrock:InvokeModelWithResponseStream` で足ります。
